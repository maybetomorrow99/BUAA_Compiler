...
MipsGenerator_old::MipsGenerator_old(string asmPath) {
...
*/
bool MipsGenerator_old::inReg(string name) {
	if (name[0] == '$')
		return false;
...
*/
int MipsGenerator_old::getRegNum(string name) {
	int addr = symTab.search(name).addr;
	int para = symTab.getCurFunc().para;
...
*/
int MipsGenerator_old::getOffset(string name) {
...
根据四元式生成Mips代码
*/
void MipsGenerator_old::genMips() {
...
字符串常量
*/
int MipsGenerator_old::mipsGlobal() {
	int index = 0;
	mipsout << ".data" << endl;
	curq = quaterList[0];
...
const int a = 6	==> CON int 6 a
*/
void MipsGenerator_old::mipsCON() {
...
int a ==> VAR int a
*/
void MipsGenerator_old::mipsVAR() {
...
int a[100] ==> ARY int 100 a
*/
void MipsGenerator_old::mipsARY() {
...
*/
void MipsGenerator_old::mipsFUNC() {
...
参与sub运算的应该都是临时变量
*/
void MipsGenerator_old::mipsCal() {
...
*/
void MipsGenerator_old::mipsPARA() {
...
x = tar(a, b) ==> CALL tar
*/
void MipsGenerator_old::mipsCALL() {
	string fname = curq.res;
	//symTab.updateCurFuncAddr(fname);
	int para = symTab.searchFunc(fname).para;
...
x = tar(a, b)  ==> PUSH a
*/
void MipsGenerator_old::mipsPUSH() {	//TODO:a一定是临时变量
...
操作类似赋值，等号左边为临时变量
*/
void MipsGenerator_old::mipsVOF() {
	string resname = curq.res;
...
*/
void MipsGenerator_old::mipsLVAR() {
	string op1name = curq.op1;
	string resname = curq.res;
...
i 和 x应该都是局部变量
*/
void MipsGenerator_old::mipsSARY() {
	string op1name = curq.op1;
	string op2name = curq.op2;
	string resname = curq.res;
...
x和i应该为局部变量
*/
void MipsGenerator_old::mipsLARY() {
	string op1name = curq.op1;
	string op2name = curq.op2;
	string resname = curq.res;
...
*/
void MipsGenerator_old::mipsPRT() {
	if (curq.op1 == "0") {
		string name = curq.res;
		int addr = -getOffset(name);
		SymbolType type = symTab.search(name).type;
...
scanf(a)	RI a
*/
void MipsGenerator_old::mipsREAD() {
	string resname = curq.res;
...
return a; ==> RET a
*/
void MipsGenerator_old::mipsRET() {
	string name = curq.res;
	int addr = -getOffset(name);
...
return;	REN
*/
void MipsGenerator_old::mipsREN() {
...
a > b ==> BGT a b lab
*/
void MipsGenerator_old::mipsBGT() {
	string op1name = curq.op1;
	string op2name = curq.op2;
...
a >= b ==> BGE a b lab
*/
void MipsGenerator_old::mipsBGE() {
	string op1name = curq.op1;
	string op2name = curq.op2;
	string label = curq.res;
	int op1addr = -getOffset(op1name);
...
a==b ==> BEQ a b lab
*/
void MipsGenerator_old::mipsBEQ() {
	string op1name = curq.op1;
	string op2name = curq.op2;
	string label = curq.res;
	int op1addr = -getOffset(op1name);
...
a <= b ==> BLE a b lab
*/
void MipsGenerator_old::mipsBLE() {
	string op1name = curq.op1;
	string op2name = curq.op2;
	string label = curq.res;
	int op1addr = -getOffset(op1name);
...
a < b ==> BLT a b lab
*/
void MipsGenerator_old::mipsBLT() {
	string op1name = curq.op1;
	string op2name = curq.op2;
	string label = curq.res;
	int op1addr = -getOffset(op1name);
...
a != b ==> BNE a b lab
*/
void MipsGenerator_old::mipsBNE() {
	string op1name = curq.op1;
	string op2name = curq.op2;
	string label = curq.res;
	int op1addr = -getOffset(op1name);
...
JUMP Lable1
*/
void MipsGenerator_old::mipsJUMP() {
...
LAB Lable1
*/
void MipsGenerator_old::mipsLAB() {
	mipsout << curq.res << ":" << endl;
}



void MipsGenerator_old::startWorking() {
	int glbIndex;
	glbIndex = mipsGlobal();
